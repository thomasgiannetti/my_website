...
Here is the link to my [LinkedIn profile](https://www.linkedin.com/in/thomas-giannetti/) which tells more about my previous experiences.
...
plot1<- plot1 +
   labs(title = "Life Expectancy In Ivory Coast over Time ",
       x = " Year ",
       y = " Life Expectancy") +
   NULL
...
plot2 <- ggplot(data = gapminder %>% 
            filter(continent == "Africa"), mapping = aes(x =  year, y= lifeExp, colour= country, group = country, fill = country))+
  geom_point() +
   geom_smooth(se = FALSE)+
  labs(title = "Life Expectancy In Africa by Country ",
       x = " Year ",
       y = " Life Expectancy") +
   NULL
...
plot3 <- ggplot(data = gapminder , mapping = aes(x = year , y = lifeExp , group = continent, colour= continent, fill = continent ))+
   geom_point()+
   geom_smooth(se = FALSE) +
...
Across the world, life expectancy has been increasing since 1952 although some the developing world still laggs behind. I believe that the population growth and GDP growth is strongly correlated to the level of life expectancy. In Africa, life expectancy was strongly increasing for most nations during the independence movements and as many countries were witnessing strong economic developments. However, during the 1980s-90s, it seems as though the end the post-colonial era led many countries to loose their momentum and life expectancy has suffered as a consequence. The near-flat curve Africa we can observe in Africa since 1990 is explained by diverging situations across the continent. I believe if we divided this analysis into sub-regions, the curves would look very different with North Africa and South Africa leading the way and West or Central Africa lagging behind.
...
ggplot(brexit_results, aes(x = leave_share)) +
  ...
  labs(title = " The Number of Seats ",
       subtitle = " Frequency Distribution of the Share of Leave ",
       x = " Leave Share ",
       y = " Seats ")
...
ggplot(brexit_results, aes(x = leave_share)) +
  geom_density()+
  labs(title = " Leave Share Distribution by Seat ",
       subtitle = " Probability Density Function of the Leave Share Variable ",
       x = " Leave Share ",
       y = " Density ")
...
ggplot(brexit_results, aes(x = leave_share)) +
  ...
  labs(title = " Cumulative Probability of Leave Share ",
...
ggplot(brexit_results, aes(x = born_in_uk, y = leave_share)) +
  ...
  theme_bw() +
  labs(title = " Relationship between Place of Birth and Leave Share ",
       subtitle = " Scatterplot and Line of Best Fit",
       x = " Born In The UK ",
       y = " Leave Share ")
  NULL
```
...
Throughout those graphs, we can see there is a higher proportion of constituencies which were in favor of Brexit rather than the opposite. We can also see there is a positive correlation if someone was born in the UK or not. It is possible that those born in the UK believed their country would do better on its own more than other individuals.
...
The mean is slightly higher than the median for all animal groups but they seem to follow a similar pattern with Horses and Cows as outliers.
...
The boxplot best communicates that variability as we have an idea of the interquartile range and at what point values can be considered as outliers to the data. The boxplot also uses standard deviation which is a measure of variability. Big animals such as cows and horses cost more in terms of single intervention however animals like cats and dogs also have a wide spread of values and can be expensive to rescue probably due to the high number of interventions and therefore the higher probability of having complicated and expensive rescues during the year.
